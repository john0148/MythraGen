...
with open('WikiArt/MetaData/images.txt', 'r') as file:
    image_paths = [line.strip() for line in file]

for i in range(len(image_paths)):
    image_paths[i] = 'WikiArt/' + "/".join(image_paths[i].split("\\"))

with open('WikiArt/MetaData/metadata.txt', 'r') as file:
...
# Load data embedding
import pickle
with open('WikiArt/Embedding/Pretrained-ViTL/WikiArt_texts_embedding.pkl', 'rb') as fp:
    texts_embedding = pickle.load(fp)
    
with open('WikiArt/Embedding/Pretrained-ViTL/WikiArt_images_embeddings.pkl', 'rb') as fp:
    images_embedding = pickle.load(fp)
       
with open('WikiArt/Embedding/Pretrained-ViTL/WikiArt_artists_embedding.pkl', 'rb') as fp:
    artists_embedding = pickle.load(fp)
    
with open('WikiArt/Embedding/Pretrained-ViTL/WikiArt_styles_embedding.pkl', 'rb') as fp:
    styles_embedding = pickle.load(fp)
    
with open('WikiArt/Embedding/Pretrained-ViTL/WikiArt_genres_embedding.pkl', 'rb') as fp:
...
with open('WikiArt/MetaData/genre_labels.pkl', 'rb') as file:
    genre_labels = pickle.load(file)

with open('WikiArt/MetaData/name_genre.pkl', 'rb') as file:
...
dataset = pd.read_csv('WikiArt/Data/wclasses.csv')
...
labels = []
for idx in indices:
    if image_paths[idx].replace("WikiArt/Data/", "") in list(df['file'].values):
        labels.append(genre_name[df[df['file'] ==  image_paths[idx].replace("WikiArt/Data/", "")]['genre'].values[0]])
    else:
